...
startSortButton.addEventListener('click', () => {
    ...
    if (size !== elements.length || isNaN(size) || elements.some(isNaN)) {
        alert('Please ensure the size matches the number of elements and they are valid numbers.');
        return;
    }
...
function mergeSort(arr, left, right) {
    if (left < right) {
        ...
        steps.push(`Divide array from index ${left} to ${mid} and ${mid + 1} to ${right}: [${arr.slice(left, mid + 1)}], [${arr.slice(mid + 1, right + 1)}]`);
...
function merge(arr, left, mid, right) {
    ...
    let i = 0, j = 0, k = left;
    const stepDescription = [`Merging: Left: [${leftArr.join(', ')}], Right: [${rightArr.join(', ')}]`];
